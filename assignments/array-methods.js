...
let allCaps = [];
// console.log(allCaps); 
runners.map((callback) => {
    allCaps.push({'first_name': callback.first_name.toUpperCase()});
})
console.log(allCaps);
...
const L_Shirts = runners.filter((callback) => {
    return callback.shirt_size === "L"
})
console.log(L_Shirts)

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
// console.log(ticketPriceTotal);
const ticketTotalPrice = runners.reduce((total, price) => {
...
// Problem 1
//use .filter()
//the company wants to see a list of runners who donated more than $200. Make a list of the runners
const bigBallers = runners.filter((callback) => {
    return callback.donation >= 200
})
console.log(bigBallers);

// Problem 2
const upperCaseAllNames = runners.map((callback) => {
    return {'first Name:': callback.first_name.toUpperCase(), 'last name': callback.last_name.toUpperCase() };
})
